...
/**
 * Portable-text manifesto. Each child span's `italicAccent` mark is detected
 * on the frontend and styled (italic + sage). Words are split for the
 * staggered fade-in via the existing `.manifesto-word` reveal.
 */
          ...
          {body.map((block, bi) =>
            block.children.map((child, ci) => {
              ...
              return (
                <span
                  key={`${bi}-${ci}`}
                  className={cn(isItalic && "italic text-sage-dark")}
                >
